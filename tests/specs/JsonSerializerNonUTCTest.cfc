component
	extends = "TestCase"
	output = false
	hint = "I test the Json Serializer component (without converting dates to UTC milliseconds)."
	{
	...
	public void function setup() {
		...
		serializer = new lib.JsonSerializer( false )
			.asInteger( "age" )
			.asDate( "dateOfBirth" )
			.asInteger( "duration" )
		...
		user = {
			...
			lastName = "Smith",
			age = 33,
			dateOfBirth = "1980/01/01 2:45 PM",
			nickName = "Trish"
		};
	...
	public void function assertUserValues( required string serializedInput ) {
		...
		assert( find( quote( "1980-01-01T14:45:00.0Z" ), serializedInput ) );
